...
class ThreedFutureDataset(object):
    ...
    def _filter_objects_by_label_style(self, label, style):
        return [oi for oi in self.objects if oi.label == label and  oi.style == style]

    def get_closest_furniture_to_box_style(self, query_label, query_size, query_style):
        objects = self._filter_objects_by_label_style(query_label, query_style)
        mses = {}
