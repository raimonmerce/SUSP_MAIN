...
def print_predicted_labels(dataset, boxes):
    ...
    #labels = object_types[float(box_id.cpu().numpy())].tolist() #Changes canvi
...
def render_scene_from_bbox_params(
    ...
    bbox_params_t = torch.cat(
        [
            boxes["class_labels"].float().cuda(),
            boxes["translations"].float().cuda(),
            boxes["sizes"].float().cuda(),
            boxes["angles"].float().cuda()
        ],
        dim=-1
    ).cpu().numpy()
