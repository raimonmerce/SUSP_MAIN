...
class BaseAutoregressiveTransformer(nn.Module):
    ...
    def start_symbol_features(self, B, room_mask):
        room_layout_f = self.fc_room_f(self.feature_extractor(room_mask.cuda()))
...
class AutoregressiveTransformer(BaseAutoregressiveTransformer):
    ...
    def _encode(self, boxes, room_mask):
        ...
        if class_labels.shape[1] == 1:
            ...
            # start token
            class_f = self.fc_class(class_labels[:, 1:].cuda())
            # Apply the positional embedding along each dimension of the
            # position property
            pos_f_x = self.pe_pos_x(translations[:, 1:, 0:1].cuda())
            pos_f_y = self.pe_pos_y(translations[:, 1:, 1:2].cuda())
            pos_f_z = self.pe_pos_z(translations[:, 1:, 2:3].cuda())
            pos_f = torch.cat([pos_f_x, pos_f_y, pos_f_z], dim=-1)

            size_f_x = self.pe_size_x(sizes[:, 1:, 0:1].cuda())
            size_f_y = self.pe_size_y(sizes[:, 1:, 1:2].cuda())
            size_f_z = self.pe_size_z(sizes[:, 1:, 2:3].cuda())
            size_f = torch.cat([size_f_x, size_f_y, size_f_z], dim=-1)

            angle_f = self.pe_angle_z(angles[:, 1:].cuda())
            X = torch.cat([class_f, pos_f, size_f, angle_f], dim=-1)
    ...
    def add_object(self, room_mask, class_label, boxes=None, device="cpu"):
        ...
        for k in start_box.keys():
            boxes[k] = torch.cat([start_box[k].cuda(), boxes[k].cuda()], dim=1)
        ...
        for k in end_box.keys():
            boxes[k] = torch.cat([boxes[k].cuda(), end_box[k].cuda()], dim=1)
